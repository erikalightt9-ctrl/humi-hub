...
import {
  ArrowLeft, Plus, Loader2, Edit2, Trash2, Package, X,
  ArrowUpRight, ArrowDownRight, SlidersHorizontal, History, Search,
  LayoutList, Table2,
} from "lucide-react";
import { CategoryBulkGrid } from "@/components/admin/inventory/CategoryBulkGrid";
...
export default function CategoryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [itemModal, setItemModal]       = useState<{ mode: "create" | "edit"; row: Item | null } | null>(null);
  ...
  const openCreateItem = () => {
    ...
    setItemModal({ mode: "create", row: null });
  };
  ...
  const handleSaveItem = async (e: FormEvent) => {
    ...
    const body = isEdit
      ...
      : {
          ...
          minThreshold: Number(itemForm.minThreshold) || 0,
          location: itemForm.location || undefined,
          initialQty: Number(itemForm.initialQty) || 0,
          supplier: itemForm.supplier || undefined,
          unitCost: itemForm.unitCost ? Number(itemForm.unitCost) : undefined,
        };

    try {
      const url = isEdit ? `/api/admin/inventory/items?id=${itemModal.row!.id}` : "/api/admin/inventory/items";
      const res = await fetch(url, {
      ...
      setBanner({ kind: "success", text: isEdit ? "Item updated" : "Item created" });
      setItemModal(null);
      await loadItems();
    } catch (err) {
  ...
  const handleDeleteItem = async (row: Item) => {
    if (!confirm(`Delete item "${row.name}"? All stock movements will also be deleted.`)) return;
  ...
  const handleSaveStock = async (e: FormEvent) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || !json.success) throw new Error(json.error || "Save failed");
      setBanner({ kind: "success", text: `${stockModal.type === "IN" ? "Added" : stockModal.type === "OUT" ? "Deducted" : "Adjusted"} stock for ${stockModal.item.name}` });
      setStockModal(null);
      await loadItems();
    } catch (err) {
      setBanner({ kind: "error", text: err instanceof Error ? err.message : "Save failed" });
    ...
    <div className="space-y-5">
        ...
        <>
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
            />
          </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((it) => {
                    ...
                    return (
                      <tr key={it.id} className="border-t border-slate-100 dark:border-slate-800 hover:bg-slate-50/60 dark:hover:bg-slate-800/50">
                        ...
                        <td className="px-3 py-2.5 text-xs text-slate-500 font-mono">{it.sku ?? "—"}</td>
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => openStock(it, "IN")} title="Stock In" className="p-1.5 rounded-md text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/40">
                              <ArrowDownRight className="h-3.5 w-3.5" />
                            </button>
                            <button onClick={() => openStock(it, "OUT")} title="Stock Out" className="p-1.5 rounded-md text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40">
                              <ArrowUpRight className="h-3.5 w-3.5" />
                            </button>
                            <button onClick={() => openStock(it, "ADJUST")} title="Adjust" className="p-1.5 rounded-md text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/40">
                              <SlidersHorizontal className="h-3.5 w-3.5" />
                            </button>
                            <button onClick={() => openHistory(it)} title="History" className="p-1.5 rounded-md text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
                              <History className="h-3.5 w-3.5" />
                            </button>
                            <button onClick={() => openEditItem(it)} title="Edit" className="p-1.5 rounded-md text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
                              <Edit2 className="h-3.5 w-3.5" />
                            </button>
                            <button onClick={() => handleDeleteItem(it)} title="Delete" className="p-1.5 rounded-md text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40">
      ...
      {itemModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" onClick={() => !savingItem && setItemModal(null)}>
          <form onSubmit={handleSaveItem} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-700 sticky top-0 bg-white dark:bg-slate-900">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
                {itemModal.mode === "edit" ? "Edit Item" : "New Item"}
              </h2>
              <button type="button" onClick={() => !savingItem && setItemModal(null)} className="p-1.5 rounded-md text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
                <X className="h-4 w-4" />
              </button>
            </div>
              ...
              {itemModal.mode === "create" && (
                <>
                  <div className="border-t border-slate-200 dark:border-slate-700 pt-3">
                    <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-2">Initial stock (optional)</p>
                    <div className="grid grid-cols-3 gap-3">
                      <Field label="Quantity"><input type="number" min="0" value={itemForm.initialQty} onChange={(e) => setItemForm({ ...itemForm, initialQty: e.target.value })} className={inputCls} /></Field>
                      <Field label="Unit Cost"><input type="number" min="0" step="0.01" value={itemForm.unitCost} onChange={(e) => setItemForm({ ...itemForm, unitCost: e.target.value })} className={inputCls} placeholder="—" /></Field>
                      <Field label="Supplier"><input value={itemForm.supplier} onChange={(e) => setItemForm({ ...itemForm, supplier: e.target.value })} className={inputCls} placeholder="—" /></Field>
                    </div>
                  </div>
                </>
              )}
            ...
            <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-slate-200 dark:border-slate-700 sticky bottom-0 bg-white dark:bg-slate-900">
              <button type="button" onClick={() => setItemModal(null)} disabled={savingItem} className="px-4 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800">Cancel</button>
              <button type="submit" disabled={savingItem} className="flex items-center gap-1.5 px-4 py-2 text-sm rounded-lg bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-400 text-white font-medium">
                ...
                {savingItem ? "Saving..." : itemModal.mode === "edit" ? "Save Changes" : "Create Item"}
              </button>
            </div>
          </form>
        </div>
      ...
      {stockModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" onClick={() => !savingStock && setStockModal(null)}>
          <form onSubmit={handleSaveStock} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-700">
              ...
              <button type="button" onClick={() => !savingStock && setStockModal(null)} className="p-1.5 rounded-md text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="p-5 space-y-3">
              <Field label={stockModal.type === "ADJUST" ? "Adjustment (+/-)" : "Quantity"} required>
                <input
                  ...
                  type="number"
                  step="any"
                  min={stockModal.type === "ADJUST" ? undefined : "0.01"}
                  value={stockForm.quantity}
                  onChange={(e) => setStockForm({ ...stockForm, quantity: e.target.value })}
                  className={inputCls}
                  placeholder={stockModal.type === "ADJUST" ? "e.g. -5 or 10" : "Qty"}
                />
              </Field>
              {stockModal.type === "IN" && (
                <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Note">
                <textarea value={stockForm.note} onChange={(e) => setStockForm({ ...stockForm, note: e.target.value })} rows={2} className={`${inputCls} resize-none`} placeholder="Optional" />
              </Field>
            </div>
            <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-slate-200 dark:border-slate-700">
              <button type="button" onClick={() => setStockModal(null)} disabled={savingStock} className="px-4 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800">Cancel</button>
      ...
      {historyFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" onClick={() => setHistoryFor(null)}>
          <div onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-700">
              <div>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Stock history</h2>
                <p className="text-xs text-slate-500 mt-0.5">{historyFor.name}</p>
              </div>
              <button onClick={() => setHistoryFor(null)} className="p-1.5 rounded-md text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
                <X className="h-4 w-4" />
              </button>
            </div>
                  ...
                  {history.map((m) => {
                    ...
                    return (
                      <li key={m.id} className="py-2.5 flex items-center justify-between gap-3">
                        ...
                        <p className={`text-sm font-semibold tabular-nums ${color} shrink-0`}>
                          {sign}{q} {m.item.unit}
                        </p>
                      </li>
                    );
...
const inputCls = "w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none";
