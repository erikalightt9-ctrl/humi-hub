...
import {
  BookOpen, Search, Users, Clock, Loader2, ExternalLink, GraduationCap, Plus,
} from "lucide-react";
...
export default function CorporateCoursesPage() {
  ...
  const filtered = courses.filter((c) => {
    ...
    return (
      (c.title.toLowerCase().includes(q) || (c.description ?? "").toLowerCase().includes(q) || (c.trainerName ?? "").toLowerCase().includes(q)) &&
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-ds-muted mt-0.5">Browse available courses and enroll your team</p>
        </div>
        <Link
              ...
              {filtered.length === 0 ? (
                <tr><td colSpan={5}>
                  <div className="py-16 text-center">
                    <BookOpen className="h-10 w-10 text-ds-muted/30 mx-auto mb-3" />
                ...
                <tr key={course.id} className="hover:bg-ds-surface/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 hidden md:table-cell">
                    ...
                    ) : <span className="text-sm text-ds-muted/50">—</span>}
                  </td>
